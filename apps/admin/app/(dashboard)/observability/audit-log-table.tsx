...
function getActionBadgeClasses(action: string): string {
  ...
  if (action.includes('create') || action.includes('success')) return 'bg-emerald-100 text-emerald-700';
                            ...
                            {data?.logs.map((log) => (
                                <tr key={log.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${getActionBadgeClasses(log.action)}`}>
                                            {log.action}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col">
                                            <span className="font-medium text-gray-900">{log.user?.name || log.userId}</span>
                                            <span className="text-xs text-gray-500">{log.user?.email}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-gray-900">
                                        {log.entity} <span className="text-xs text-gray-500">({log.entityId.slice(0, 8)}...)</span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <Dialog>
