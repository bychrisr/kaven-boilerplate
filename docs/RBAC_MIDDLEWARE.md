# RBAC Middleware - Guia de Uso

## 📋 Visão Geral

O **RBAC Middleware** (Role-Based Access Control) implementa controle de acesso baseado em roles com hierarquia de permissões.

---

## 🎭 Roles Disponíveis

### SUPER_ADMIN (Nível 3)

- **Acesso total** a todos os tenants
- Pode gerenciar tenants (criar, atualizar, deletar)
- Pode gerenciar usuários de qualquer tenant
- Bypass de restrições de tenant

### TENANT_ADMIN (Nível 2)

- **Acesso total** ao seu próprio tenant
- Pode gerenciar usuários do seu tenant
- Pode visualizar e gerenciar recursos do tenant
- Não pode acessar outros tenants

### USER (Nível 1)

- **Acesso limitado**
- Pode visualizar e atualizar apenas seus próprios dados
- Não pode gerenciar outros usuários
- Acesso read-only a recursos compartilhados

---

## 🔒 Middlewares Disponíveis

### 1. `authMiddleware`

Verifica JWT e injeta `request.user`

```typescript
import { authMiddleware } from '../middleware/auth.middleware';

fastify.get('/api/users/me', {
  preHandler: [authMiddleware],
  handler: userController.getCurrent,
});
```

### 2. `requireRole(allowedRoles)`

Requer uma das roles especificadas

```typescript
import { requireRole } from '../middleware/rbac.middleware';

fastify.get('/api/users', {
  preHandler: [authMiddleware, requireRole(['TENANT_ADMIN', 'SUPER_ADMIN'])],
  handler: userController.list,
});
```

### 3. `requireSuperAdmin`

Atalho para `requireRole(['SUPER_ADMIN'])`

```typescript
import { requireSuperAdmin } from '../middleware/rbac.middleware';

fastify.post('/api/tenants', {
  preHandler: [authMiddleware, requireSuperAdmin],
  handler: tenantController.create,
});
```

### 4. `requireTenantAdmin`

Atalho para `requireRole(['TENANT_ADMIN', 'SUPER_ADMIN'])`

```typescript
import { requireTenantAdmin } from '../middleware/rbac.middleware';

fastify.delete('/api/users/:id', {
  preHandler: [authMiddleware, requireTenantAdmin],
  handler: userController.delete,
});
```

### 5. `requireResourceOwnership(paramName)`

Verifica se usuário pode acessar recurso específico

```typescript
import { requireResourceOwnership } from '../middleware/rbac.middleware';

fastify.put('/api/users/:id', {
  preHandler: [authMiddleware, requireResourceOwnership('id')],
  handler: userController.update,
});
```

**Regras:**

- `SUPER_ADMIN`: pode acessar qualquer recurso
- `TENANT_ADMIN`: pode acessar recursos do seu tenant
- `USER`: só pode acessar seus próprios recursos

### 6. `requireTenantAccess`

Verifica se usuário pode acessar tenant solicitado

```typescript
import { requireTenantAccess } from '../middleware/rbac.middleware';

fastify.get('/api/data', {
  preHandler: [authMiddleware, requireTenantAccess],
  handler: dataController.list,
});
```

---

## 📊 Endpoints Protegidos

### Users (`/api/users`)

| Endpoint | Método | Permissão Requerida         |
| -------- | ------ | --------------------------- |
| `/`      | GET    | TENANT_ADMIN ou SUPER_ADMIN |
| `/me`    | GET    | Autenticado                 |
| `/:id`   | GET    | Owner ou TENANT_ADMIN       |
| `/`      | POST   | TENANT_ADMIN                |
| `/:id`   | PUT    | Owner ou TENANT_ADMIN       |
| `/:id`   | DELETE | TENANT_ADMIN                |

### Tenants (`/api/tenants`)

| Endpoint | Método | Permissão Requerida         |
| -------- | ------ | --------------------------- |
| `/`      | GET    | TENANT_ADMIN ou SUPER_ADMIN |
| `/:id`   | GET    | TENANT_ADMIN                |
| `/`      | POST   | SUPER_ADMIN                 |
| `/:id`   | PUT    | SUPER_ADMIN                 |
| `/:id`   | DELETE | SUPER_ADMIN                 |

---

## 🧪 Testes

### 1. Login e Obter Token

```bash
# Login como USER
curl -X POST http://localhost:8000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "password": "password123"
  }'

# Resposta:
{
  "accessToken": "eyJhbGciOiJIUzI1NiIs...",
  "refreshToken": "1234567890.abc123",
  "user": {
    "id": "uuid",
    "email": "user@example.com",
    "role": "USER"
  }
}
```

### 2. Testar Acesso Negado (USER tentando listar usuários)

```bash
curl -H "Authorization: Bearer <user-token>" \
  http://localhost:8000/api/users

# Resposta esperada: 403 Forbidden
{
  "error": "Acesso negado",
  "message": "Requer uma das roles: TENANT_ADMIN, SUPER_ADMIN",
  "userRole": "USER"
}
```

### 3. Testar Acesso Permitido (TENANT_ADMIN listando usuários)

```bash
curl -H "Authorization: Bearer <admin-token>" \
  http://localhost:8000/api/users

# Resposta esperada: 200 OK
{
  "users": [...],
  "pagination": {...}
}
```

### 4. Testar Resource Ownership (USER acessando próprio perfil)

```bash
# Permitido: próprio ID
curl -H "Authorization: Bearer <user-token>" \
  http://localhost:8000/api/users/<own-user-id>

# Negado: ID de outro usuário
curl -H "Authorization: Bearer <user-token>" \
  http://localhost:8000/api/users/<other-user-id>

# Resposta: 403 Forbidden
{
  "error": "Acesso negado",
  "message": "Você só pode acessar seus próprios dados"
}
```

---

## 🔧 Helpers Disponíveis

```typescript
import {
  isSuperAdmin,
  isTenantAdmin,
  canManageUsers,
  canManageTenants,
} from '../middleware/rbac.middleware';

// Verificar role
if (isSuperAdmin(request.user)) {
  // Lógica para super admin
}

// Verificar permissão
if (canManageUsers(request.user)) {
  // Pode gerenciar usuários
}
```

---

## 📝 Exemplo Completo

```typescript
import { FastifyInstance } from 'fastify';
import { authMiddleware } from '../middleware/auth.middleware';
import {
  requireSuperAdmin,
  requireTenantAdmin,
  requireResourceOwnership,
} from '../middleware/rbac.middleware';

export async function myRoutes(fastify: FastifyInstance) {
  // Público (sem auth)
  fastify.get('/public', publicHandler);

  // Autenticado (qualquer role)
  fastify.get('/me', {
    preHandler: [authMiddleware],
    handler: meHandler,
  });

  // Apenas TENANT_ADMIN ou superior
  fastify.get('/admin', {
    preHandler: [authMiddleware, requireTenantAdmin],
    handler: adminHandler,
  });

  // Apenas SUPER_ADMIN
  fastify.post('/super', {
    preHandler: [authMiddleware, requireSuperAdmin],
    handler: superHandler,
  });

  // Owner ou TENANT_ADMIN
  fastify.put('/:id', {
    preHandler: [authMiddleware, requireResourceOwnership('id')],
    handler: updateHandler,
  });
}
```

---

## ⚠️ Erros Comuns

### 401 Unauthorized

```json
{
  "error": "Token não fornecido",
  "message": "Header Authorization com Bearer token é obrigatório"
}
```

**Solução:** Adicionar header `Authorization: Bearer <token>`

### 403 Forbidden

```json
{
  "error": "Acesso negado",
  "message": "Requer uma das roles: TENANT_ADMIN, SUPER_ADMIN",
  "userRole": "USER"
}
```

**Solução:** Usuário não tem permissão suficiente

### 401 Token Inválido

```json
{
  "error": "Token inválido",
  "message": "Token JWT inválido ou expirado"
}
```

**Solução:** Fazer login novamente ou usar refresh token

---

## 🚀 Boas Práticas

1. **Sempre use `authMiddleware` primeiro** antes de RBAC middlewares
2. **Combine middlewares** para controle granular
3. **Use helpers** para lógica condicional no código
4. **Documente permissões** de cada endpoint
5. **Teste com diferentes roles** durante desenvolvimento

---

**Versão:** 1.0.0  
**Última atualização:** 2025-12-19
